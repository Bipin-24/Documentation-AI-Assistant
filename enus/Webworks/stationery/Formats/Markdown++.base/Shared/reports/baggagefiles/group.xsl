...
>
     ...
     <xsl:for-each select="$GlobalInput[1]">
      ...
      <xsl:if test="count($VarGroupFilesOfType[1]) = 1">
       ...
       <xsl:if test="not($VarUpToDate)">
        <xsl:variable name="VarResultAsTreeFragment">
         ...
         <xsl:choose>
          <xsl:when test="$GlobalCreateStandaloneGroup='true'">
            ...
            <!--          -->
            <xsl:if test="not(wwprogress:Abort())">
             <xsl:copy-of select="wwexsldoc:LoadXMLWithoutResolver($VarGroupFilesOfType[1]/@path, false())/wwreport:Report/*" />
            </xsl:if>
          </xsl:when>
          <xsl:otherwise>
        ...
        <xsl:variable name="VarCleanResultAsTreeFragment">
         <xsl:call-template name="Remove-Duplicates">
          <xsl:with-param name="ParamTree" select="$VarResultAsTreeFragment" />
         </xsl:call-template>
        </xsl:variable>
        <xsl:if test="not(wwprogress:Abort())">
         <xsl:variable name="VarWriteResult" select="wwexsldoc:Document($VarCleanResultAsTreeFragment, $VarPath, 'utf-8', 'xml', '1.0', 'yes')" />
        </xsl:if>
       </xsl:if>
 ...
 <xsl:template name="Remove-Duplicates">
  <xsl:param name="ParamTree" />
  
  <xsl:variable name="VarAsTree" select="msxsl:node-set($ParamTree)"/>
  <wwreport:Report version="1.0">
   <xsl:for-each select="$VarAsTree/wwreport:Report/*">
    <xsl:variable name="VarEntry" select="." />
    <xsl:if test="count(preceding-sibling::wwreport:Entry/wwreport:Description[text()=$VarEntry/wwreport:Description/text()]) = 0">
